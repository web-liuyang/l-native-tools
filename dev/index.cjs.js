...
function typeOf(origin) {
  return Object.prototype.toString.call(origin).slice(8, -1).toLowerCase();
}
/**
...
function straightDistance(origin, target) {
  ...
  var PI = Math.PI,
      ...
      sin = Math.sin,
      cos = Math.cos,
      round = Math.round; // 获取经纬度

  var _a = __read(origin.split(','), 2),
      lon1 = _a[0],
      lat1 = _a[1];

  var _b = __read(target.split(','), 2),
      lon2 = _b[0],
      lat2 = _b[1];
...
function currency(method, numberArr) {
  var arr = numberArr.map(function (item) {
  ...
  switch (method) {
    case '+':
      num = arr.reduce(function (total, item) {
        return total + item;
      }) / 100;
      break;
...
function debounce(fn, delay, immediate) {
  ...
  return function () {
    // @ts-ignore
    var ctx = this;
    var args = arguments; // 如果timeout存在那么取消延时器

    if (timeout) clearTimeout(timeout); // 判断是否首次需要执行
...
function throttle(fn, delay) {
  ...
  return function () {
    ...
    remaining = delay - (curTime - startTime),
        // 设定的delay与[上一次被调用的时间与现在的时间间隔]的差值
    // @ts-ignore
    ctx = this,
        // 上下文对象
    ...
    timeout && clearTimeout(timeout); // // 假如距离上一次执行此函数的时间已经超过了设定的delay，则执行
